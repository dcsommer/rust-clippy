...
/// Resolves the lint names given in the `allow-in-tests` and `check-in-tests` configurations
/// and hands them to `clippy_utils`, which consults them when a diagnostic is emitted from test
/// code.
///
